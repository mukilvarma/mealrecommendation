...
@Service
public class GooglePlacesServiceImpl implements RestaurantSearchService, MenuItemService {
    ...
    public List<MenuItem> getMenuItemsForRestaurant(String placeId) {
        // Mock implementation for extracting menu items
        // This should ideally call a real menu API from Zomato or some other APIs - paid
        return extractMenuItems(null); // Pass the real response here
    }

    @Override
    public List<MenuItem> extractMenuItems(JsonNode restaurantDetails) {
        // Mock data for demonstration
        return List.of(
                new MenuItem("Grilled Chicken Salad", "A healthy salad with grilled chicken and mixed greens.", 12.99, 350, 30.5, 10.0, 20.0),
                new MenuItem("Veggie Pizza", "A delicious pizza topped with fresh vegetables and cheese.", 14.99, 700, 15.0, 25.0, 85.0),
                new MenuItem("Pasta Primavera", "Pasta with a mix of seasonal vegetables in a light sauce.", 13.49, 600, 20.0, 12.0, 75.0),
                new MenuItem("Beef Tacos", "Tacos filled with seasoned beef and fresh toppings.", 11.99, 500, 27.0, 22.0, 40.0),
                new MenuItem("Chocolate Cake", "Rich chocolate cake topped with creamy frosting.", 6.99, 200, 5.0, 15.0, 40.0)
        );
    }
